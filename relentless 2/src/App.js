...
// Day-aware MIT text — what actually moves the needle TODAY
function getDayMIT(todayDay, savedTasks) {
  // If user has dumped tasks today, use AI result
  if (savedTasks?.mit) return savedTasks.mit;
  // Otherwise use day-based default
  const defaults = {
    Content: 'Write + record one reel today',
    SaaS:    'Work on WiserMoney / SaaS build',
    BedBox:  'Clear all BedBox ops tasks today',
    Rest:    'Rest. Weekly review at 8 PM.',
  };
  return defaults[todayDay?.primary] || 'Do the one thing that moves the needle';
}

// Day-aware micro start
function getDayMicroStart(todayDay, savedTasks) {
  if (savedTasks?.micro_start) return savedTasks.micro_start;
  const defaults = {
    Content: 'Open the script doc and write the hook line',
    SaaS:    'Open the codebase and pick one feature to ship',
    BedBox:  'List all pending BedBox tasks in one place',
    Rest:    'Close all work tabs. You\'re off.',
  };
  return defaults[todayDay?.primary] || 'Open the relevant doc and start';
}
  ...
  const handleTick = useCallback((blockId, streakType) => {
    ...
    if (nowDone) {
      ...
      if (streakType === 'rec') {
        bumpStreak('rec');
        markWin('rec');
        showToast(`Recording streak: ${streaks.rec + 1} days 🔥`, true);
      }
      if (streakType === 'gym') {
        bumpStreak('gym');
        markWin('gym');
        showToast(`Gym streak: ${streaks.gym + 1} days 💪`, true);
      }
    }
  }, [isBlockDone, toggleBlock, celebrate, bumpStreak, markWin, showToast, streaks]);
